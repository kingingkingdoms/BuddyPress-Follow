...
function bp_follow_action_start() {
	...
	if ( bp_follow_is_following( array( 'leader_id' => bp_displayed_user_id(), 'follower_id' => bp_loggedin_user_id() ) ) ) {
		bp_core_add_message( sprintf( __( 'You have already added %s to your activity wall.', 'bp-follow' ), bp_get_displayed_user_fullname() ), 'error' );

	} else {
		if ( ! bp_follow_start_following( array( 'leader_id' => bp_displayed_user_id(), 'follower_id' => bp_loggedin_user_id() ) ) ) {
			bp_core_add_message( sprintf( __( 'There was a problem when trying to add %s to your activity wall, please try again.', 'bp-follow' ), bp_get_displayed_user_fullname() ), 'error' );
		} else {
			bp_core_add_message( sprintf( __( 'You have now added %s to your activity wall.', 'bp-follow' ), bp_get_displayed_user_fullname() ) );
		}
	}
...
function bp_follow_action_stop() {
	...
	if ( ! bp_follow_is_following( array( 'leader_id' => bp_displayed_user_id(), 'follower_id' => bp_loggedin_user_id() ) ) ) {
		bp_core_add_message( sprintf( __( 'You have not added %s to your activity wall.', 'bp-follow' ), bp_get_displayed_user_fullname() ), 'error' );

	} else {
		if ( ! bp_follow_stop_following( array( 'leader_id' => bp_displayed_user_id(), 'follower_id' => bp_loggedin_user_id() ) ) ) {
			bp_core_add_message( sprintf( __( 'There was a problem when trying to add %s to your activity wall, please try again.', 'bp-follow' ), bp_get_displayed_user_fullname() ), 'error' );
		} else {
			bp_core_add_message( sprintf( __( 'You have removed %s from your activity wall.', 'bp-follow' ), bp_get_displayed_user_fullname() ) );
		}
	}
	...
	// failed follow
		...
		if ( bp_follow_is_following( array( 'leader_id' => $_POST['uid'], 'follower_id' => bp_loggedin_user_id() ) ) ) {
			$output = bp_get_button( array_merge(
				array( 'link_text' => __( 'Already added', 'bp-follow' ) ),
				$args
			) );
		} else {
			$output = bp_get_button( array_merge(
				array( 'link_text' => __( 'Error adding user', 'bp-follow' ) ),
				$args
			) );
		}
	}
...
function bp_follow_ajax_action_stop() {
	...
	// failed unfollow
		...
		if ( ! bp_follow_is_following( array( 'leader_id' => $_POST['uid'], 'follower_id' => bp_loggedin_user_id() ) ) ) {
			$output = bp_get_button( array_merge(
				array( 'link_text' => __( 'Not added', 'bp-follow' ) ),
				$args
			) );

		} else {
			$output = bp_get_button( array_merge(
				array( 'link_text' => __( 'Error removing user', 'bp-follow' ) ),
				$args
			) );
