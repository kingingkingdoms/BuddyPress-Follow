...
function bp_follow_add_activity_tab() {
	...
	if ( empty( $counts['following'] ) )
		return false;
	?>
	<li id="activity-following"><a href="<?php echo bp_loggedin_user_domain() . BP_ACTIVITY_SLUG . '/' . BP_FOLLOWING_SLUG . '/' ?>" title="<?php _e( 'The public activity for all the brands, organizations, famous people, and Millionaire’s Digest accounts you have added to your activity.', 'bp-follow' ) ?>"><?php printf( __( 'Added <span>%d</span>', 'bp-follow' ), (int)$counts['following'] ) ?></a></li><?php
}
...
function bp_follow_add_following_tab() {
	...
	if ( empty( $counts['following'] ) )
		return false;
	?>
	<li id="members-following"><a href="<?php echo bp_loggedin_user_domain() . BP_FOLLOWING_SLUG ?>"><?php printf( __( 'Added <span>%d</span>', 'bp-follow' ), $counts['following'] ) ?></a></li><?php
}
	...
	if ( $untranslated_text == 'Sorry, there was no activity found. Please try a different filter.' ) {
		if ( ! bp_is_user() || bp_is_my_profile() ) {
			...
			if ( $follow_counts['following'] ) {
				return __( "You have added some accounts, but they haven't posted anything yet.", 'bp-follow' );
			} else {
				return __( "You have not added any accounts yet.", 'bp-lists' );
			}
		} else {
			global $bp;

			if ( ! empty( $bp->displayed_user->total_follow_counts['following'] ) ) {
				return __( "This user has added some accounts, but none of them have posted anything yet.", 'bp-follow' );
			} else {
				return __( "This user hasn't added any accounts yet.", 'bp-follow' );
			}
		}
