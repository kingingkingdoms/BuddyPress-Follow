...
function bp_follow_format_notifications( $action, $item_id, $secondary_item_id, $total_items, $format = 'string' ) {
	...
	switch ( $action ) {
		case 'new_follow':
			...
			if ( 1 == $total_items ) {
				$text = sprintf( __( '%s added you to their activity wall', 'bp-follow' ), bp_core_get_user_displayname( $item_id ) );
				$link = bp_core_get_user_domain( $item_id  ) . '?bpf_read';

			} else {
				$text = sprintf( __( '%d people added you to their activity wall', 'bp-follow' ), $total_items );
...
?>
	...
	<table class="notification-settings" id="follow-notification-settings">
		...
		<tbody>
			<tr>
				<td></td>
				<td><?php _e( 'A member adds you to their activity wall', 'bp-follow' ) ?></td>
...
function bp_follow_new_follow_email_notification( $args = '' ) {
	...
	$subject = '[' . wp_specialchars_decode( bp_get_option( 'blogname' ), ENT_QUOTES ) . '] ' . sprintf( __( '%s has added you to their activity', 'bp-follow' ), $follower_name );

	$message = sprintf( __(
'%s added you to their activity wall.
